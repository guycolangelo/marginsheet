...
- **TEST EVERY SWEEP AGAINST A KNOWN-POSITIVE BEFORE BELIEVING ITS NEGATIVE.** The instrument-verification rule's cleanest instance, because here the instrument reported **clean** while structurally unable to see the very thing it was built to find.
  ...
  **A COMPARISON CAN ALSO BE DEGENERATE BECAUSE THE THING IT COMPARES WAS NEVER FETCHED**, which is the same failure with the instrument intact and its input empty. On 22 Aug 2026 a diff of two branches' open-item ids reported **no additions and no removals in either direction**, which is a clean bill of health. `gh pr checkout` had failed silently, so both sides of the comparison were `main`, and **the diff was correct about two identical inputs.** Re-run against the real branch it showed one addition per side.

  It is the empty-fixture rule pointed at a diff: **an assertion over two references that turn out to be one reference is not weak evidence, it is zero.** The tell is that a difference-shaped question answered "no difference" without anything establishing that two different things were being compared, and the cheap guard is to print what is being compared -- the counts, the refs -- rather than only the difference.
