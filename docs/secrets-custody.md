...
- **15 Aug 2026**: every Worker environment's `NEON_DATABASE_URL` was issued for `neondb_owner`, a role holding `BYPASSRLS`, so the application would have read past every household isolation policy. Found by the M3 spike into whether Neon permits `BYPASSRLS`, which also proved migration 0008's stated reasoning wrong (`FORCE` never filtered the owner, because `BYPASSRLS` supersedes it). No data was exposed: no application code queried the database yet. Remediation: migration 0009 granted `LOGIN` to `marginsheet_app`, and `/debug/db-identity` plus `services/api/test/db-identity.test.ts` verify the deployed reality rather than the configuration. **This entry originally recorded the remediation as complete on all six Workers. It was not.** See the two entries below: the reissue silently stored empty strings, and the migration granting `LOGIN` had never been applied to any long-lived branch, so the role did not exist to be reissued for. Complete on dev only as of 15 Aug 2026; staging and production are open.
- **15 Aug 2026**: all six Worker `NEON_DATABASE_URL` secrets were overwritten with the **empty string**. The reissue command piped `scripts/app-db-url.mts` into `wrangler secret put`, but `tsx` and `postgres` were declared only in `packages/schema` and the helper was documented to run from the repo root, so `pnpm exec` failed, the pipe delivered nothing, and `wrangler secret put` stored the empty result and reported success. Every environment continued to report healthy at `/health` for hours, because nothing asked the database anything. Found by adding a `present` versus empty distinction to `/debug/db-identity`. Remediation: `tsx` and `postgres` added to root `devDependencies`; the reissue now validates the value matches `postgresql://marginsheet_app:` before storing; `db-identity.test.ts` asserts the positive (`current_user` IS `marginsheet_app` AND `bypassrls` is false) against the live edge in all three environments. Dev is green; staging and production still hold empty values.
- **15 Aug 2026**: no M1 migration had ever been applied to any long-lived Neon branch. `main`, `staging` and `dev` each held zero user tables across both the `marginsheet` and `neondb` databases, and neither `marginsheet_app` nor `marginsheet_sync` existed anywhere. `ci.yml` migrates only the ephemeral per-PR branch, which `neon-pr-cleanup.yml` destroys; `deploy.yml` has no migration step at any stage. Ten PRs merged green and M1 was accepted on that evidence. No data was at risk, since there was no data and no schema. Remediation: dev migrated by hand (10 migrations applied) to unblock the identity check; a migrate-before-deploy step for staging and production is Guy's ruling, tracked as `long-lived-branches-unmigrated` in the invariant manifest.
